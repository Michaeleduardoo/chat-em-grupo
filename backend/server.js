...
app.post('/messages', (req, res) => {
  try {
    ...
    if (!content || !sender || !type) {
      return res.status(400).json({ error: 'Campos obrigatórios: content, sender, type' });
    }
...
app.put('/messages/:id', (req, res) => {
  try {
    ...
    if (!content || !sender) {
      ...
      return res.status(400).json({ error: 'Campos obrigatórios: content, sender' });
    }
...
app.get('/stats', (req, res) => {
  try {
    ...
    const fileMessages = messages.filter(msg => msg.type === 'image' || msg.type === 'audio');
    const imageFiles = fileMessages.filter(msg => msg.type === 'image').length;
...
setInterval(() => {
  ...
  for (const [socketId, user] of onlineUsers.entries()) {
    const lastSeen = user.lastSeen ? new Date(user.lastSeen) : new Date(user.joinedAt);
